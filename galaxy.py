...
tab_tirennx=[]
img_tirennx= pygame.image.load("tirenn.png").convert_alpha()
...
imgennx = pygame.image.load("photo2.png").convert_alpha()
...
def corps():
	...
	tab_tirennx=[]
	recttirennx = img_tirennx.get_rect()

	ennx = imgennx.get_rect()
	ennx.x = largeur
	ennx.y = hauteur+100
...
#vague ennemis
		if temps%50 ==0 and temps > 200  and score < 10:
			rectenn = imgenn.get_rect()
...
#tir ennemis
		if temps%20 == 0 and temps > 250:
				recttirennx = img_tirennx.get_rect()
				recttirennx.centerx=ennx.centerx
				recttirennx.y=ennx.bottom
				tab_tirennx.append(recttirennx)
#vitesse tirs vaisseau ennemi
		for tirenn in tab_tirenn:
			tirenn.y+=15
#vitesse tirs vaisseau ennemi
		for tirennx in tab_tirennx:
			tirennx.y+=10
...
#collision entre les tirs ennemis et le vaisseau
		for recttirennx in tab_tirennx:
			if recttirennx.colliderect(rectv):
				vie=vie-1
				vitessevaisseau=10
				recttirennx.y = hauteur
...
#tir ennemis
		...
		if score > 1 and viie>0  :
			if score > 1 and time==0 and viie>0 :
				ennx.x = largeur/2
				ennx.y = hauteur/4
			time +=1
			ennx.x =  math.cos (math.pi / 24 * time) + ennx.x
			ennx.y = math.sin (math.pi / 12 * time) + ennx.y
			fenetre.blit(imgennx, ennx)
